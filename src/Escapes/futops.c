...
static retCode pollCellFuture(futurePo ft, void* cl, void* cl2) {
...
ValueReturn s__cell_future(enginePo P, termPo f) {
  assert(isCell(f));
  return normalReturn((termPo) makeFuture(f, pollCellFuture, Null, Null));
}

ValueReturn s__futureIsResolved(enginePo P, termPo f) {
  return normalReturn(futureIsResolved(C_FUTURE(f)) ? trueEnum : falseEnum);
}
