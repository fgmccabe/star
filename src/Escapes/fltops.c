...
ReturnStatus g__flt2str(heapPo h, termPo a1, termPo a2, termPo a3, termPo a4) {
  double Arg = floatVal(a1);
  int precision = (int) integerVal(a2);
  codePoint mdc = charVal(a3);
  FloatDisplayMode mode = (mdc == 'f' ? fractional : mdc == 's' ? scientific : general);
  char buff[64];

  formatDouble(buff, NumberOf(buff), Arg, mode, precision, a4 == trueEnum ? True : False);
