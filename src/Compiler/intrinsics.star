star.compiler.intrinsics{
  ...
  intrinsic(Es) => case Es in {
    ...
    | "_fiber" => .some((.allType(.kVar("r"),.allType(.kVar("s"),.funType(.tupleType([.funType(.tupleType([.tpExp(.tpExp(.tpFun("fiber",2),.kVar("r")),.kVar("s"))]),.kVar("r"),.voidType)]),.tpExp(.tpExp(.tpFun("fiber",2),.kVar("r")),.kVar("s")),.voidType))),([Rs, V0],Lb) => .iFiber(Rs, V0)))  -- create a new fiber
