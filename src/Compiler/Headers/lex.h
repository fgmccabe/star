...
typedef struct _token_ {
  ...
  union {
    ...
    double f;			/* or a float */
    char *text;		/* or some text */
    char ch;                 // For a character literal
  } tk;
...
extern char *tkText(tokenPo tok);
extern char tkChar(tokenPo tok);
