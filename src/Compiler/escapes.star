...
star.compiler.escapes{
  ...
  escapeType(Es) => case Es in {
    ...
    | "_jit_compile" => .some(.allType(.kVar("a"),.allType(.kVar("b"),.constrainedType(.tpExp(.tpExp(.tpFun("=>",2),.tupleType([.tpExp(.tpExp(.tpFun("=>",2),.tupleType([.kVar("a")])),.kVar("b"))])),.tupleType([])),.raisEs(.nomnal("errorCode"))))))
    | _ default => .none
  }
