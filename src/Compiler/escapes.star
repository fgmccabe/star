star.compiler.escapes{
  ...
  escapeType(Es) => case Es in {
    ...
    | "sqrt" => .some(.constrainedType(.tpExp(.tpExp(.tpFun("=>",2),.tupleType([.nomnal("float")])),.nomnal("float")),.throWs(.nomnal("errorCode"))))
    | "exp" => .some(.constrainedType(.tpExp(.tpExp(.tpFun("=>",2),.tupleType([.nomnal("float")])),.nomnal("float")),.throWs(.nomnal("errorCode"))))
