star.compiler.checker{
  ...
  typeOfExp(A,Tp,Env,Path) where (Lc,K,As) ?= isInvoke(A) => valof{
    if [Arg].=As then{
      ATp = newTypeVar("ɑ");
      KTp = contType(ATp,Tp);
      KI = typeOfExp(K,KTp,Env,Path);
      Ar = typeOfExp(Arg,ATp,Env,Path);

      if traceCanon! then{
	showMsg("invoke: K = $(KI), A = $(Ar)");
      };
      
      valis .invoke(Lc,KI,Ar,Tp)
    }
    else{
      reportError("continuations have exactly one argument, not $(As)",Lc);
      valis .anon(Lc,Tp)
    }
  }
