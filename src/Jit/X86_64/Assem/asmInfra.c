...
static retCode showX64Reg(ioPo f, void* data, long depth, long precision, logical alt) {
  x64Reg reg = *(x64Reg*)data;
  if (reg >= RAX && reg <= R15) {
    return outMsg(f, "%s", regNames[reg]);
  }
  return outMsg(f, "reg?%d", reg);
}
...
static retCode showAssemLbl(ioPo f, void* data, long depth, long precision, logical alt) {
  ...
  if (isLabelDefined(lbl))
    return outMsg(f, "L%lld[%llu]", (long long)lbl->lblNo, (unsigned long long)labelTgt(lbl));
  else
    return outMsg(f, "L%lld?", (long long)lbl->lblNo);
}
...
static retCode showFlexOp(ioPo f, void* data, long depth, long precision, logical alt) {
  ...
  switch (flex->mode) {
    case Reg:
      return outMsg(f, "%s", regNames[flex->op.reg]);
    case Immediate:
      return outMsg(f, "$%lld", (long long)flex->op.imm);
    case Based:
      return outMsg(f, "%d(%s)", flex->op.based.disp, regNames[flex->op.based.base]);
    case Indexed:
      return outMsg(f, "%d(%s,%s,%d)", flex->op.indexed.disp, regNames[flex->op.indexed.base], regNames[flex->op.indexed.index], flex->op.indexed.scale);
    case Labeled:
      return outMsg(f, "%E", flex->op.lbl);
  }
  return Ok;
}
...
assemCtxPo createCtx() {
  ...
  ctx->size = 1024;
  ctx->pc = 0;
  ctx->labels = allocArray(sizeof(codeLblPo), 16, True);
...
retCode cleanupLabels(assemCtxPo ctx) {
  if (ctx->labels != Null) {
    ctx->labels = eraseArray(ctx->labels, clrLblProc, ctx);
  }
  return Ok;
}
...
codeLblPo newLabel(assemCtxPo ctx) {
  ...
  lbl->lblNo = ctx->lblCnt++;
  lbl->pc = undefinedPc;
  lbl->refs = Null;

  appendEntry(ctx->labels,&lbl);
