...
static localVarPo pushFlex(codeGenPo state, int32 pc, varDescPo desc, FlexOp src);
...
retCode jitInstructionsA(jitCompPo jit, methodPo mtd, char *errMsg, integer msgLen) {
  ...
  CodeGenState state = {
    .mtd = mtd, .analysis = &analysis, .locals = locals, .numLocals = numSlots, .jit = jit, .top = 0, .stack = stack
  };
...
retCode jitSpecialInstructionsA(jitCompPo jit, methodPo mtd, int32 depth) {
  ...
  CodeGenState state = {
    .mtd = mtd, .analysis = &analysis, .locals = locals, .numLocals = numSlots, .jit = jit, .stack = stack, .top = 0
  };
  populateLocals(&state, mtdArity(mtd), mtdArity(mtd));
  ...
  for (int32 pc = from; ret == Ok && pc < endPc; pc++) {
    ...
    switch (code[pc].op) {
      ...
      case OCall: {
        ...
        loadRegister(state, lam->src, temp);
...
int32 loadArguments(codeGenPo state, int32 pc, int32 arity) {
  ...
  int32 minOffset = stashLiveLocals(state, pc + 1); // save vars that will be live after the call
...
void pushVar(codeGenPo state, int32 pc, localVarPo v) {
  assert(v!=Null && state->top<state->numLocals);
...
localVarPo pushFlex(codeGenPo state, int32 pc, varDescPo desc, FlexOp src) {
  localVarPo lcl = allocateLocal(state, pc);
