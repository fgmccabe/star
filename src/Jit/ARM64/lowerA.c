...
static void storeVar(codeGenPo state, int32 pc, FlexOp val, localVarPo phiVar);
...
void storeVar(codeGenPo state, int32 pc, FlexOp val, localVarPo phiVar) {
  if (phiVar->stashed) {
...
static localVarPo findLocal(codeGenPo state, int32 lx) {
  ...
  for (int32 ix = 0; ix < state->numLocals; ix++) {
    localVarPo lcl = &state->locals[ix];
    if (lcl->live && lcl->varDesc == varDesc)
      return lcl;
  }
  localVarPo lcl = allocateLocal(state);
