...
retCode jitSpecialInstructionsA(jitCompPo jit, methodPo mtd, int32 depth) {
  ...
  CodeGenState state = {
    .mtd = mtd, .analysis = &analysis, .locals = locals, .numLocals = numSlots, .jit = jit, .stack = stack
  };
  populateLocals(&state, mtdArity(mtd), mtdArity(mtd));
...
retCode jitBlock(blockPo block, codeGenPo state, insPo code, int32 from, int32 endPc) {
  ...
  for (int32 pc = from; ret == Ok && pc < endPc; pc++){
    ...
    case Underflow: {
      ...
      invokeIntrinsic(state, pc, (runtimeFn)detachDropStack, 3, (FlexOp[]){RG(PR), RG(STK), RG(RTV)});
