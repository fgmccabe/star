...
static void storeToPhiVar(codeGenPo state, int32 pc, FlexOp val, localVarPo phiVar);
static armReg getTempReg(codeGenPo state, int32 pc);
...
static void maybeRetireVar(codeGenPo state, int32 vx, int32 pc);
...
retCode jitBlock(blockPo block, codeGenPo state, insPo code, int32 from, int32 endPc) {
  ...
  for (int32 pc = from; ret == Ok && pc < endPc; pc++) {
    ...
    switch (code[pc].op) {
      ...
      case Abort: {
        // abort with message
        armReg loc = getTempReg(state, pc);
      ...
      case Call: {
        ...
        pushVar(state, pc, markVarStart(state,RG(RTN), pc));
        continue;
      }
      case XCall: {
        int32 key = code[pc].fst;
      ...
      case OCall: {
        ...
        armReg temp = getTempReg(state, pc);
      ...
      case Result: {
        ...
        FlexOp val = popFlex(state, pc);
        storeToPhiVar(state, pc, val, phiVar);
      ...
      case Fiber: {
        ...
        localVarPo nv = markVarStart(state,RG(X0), pc);
        pushVar(state, pc, nv);
        continue;
      }
      case Suspend: {
        armReg tmp = getTempReg(state, pc);
        ...
        bind(rtn);
        releaseReg(jit, tmp);
        pushVar(state, pc, markVarStart(state,RG(X0), pc));
        continue;
      }
      case Resume: {
        FlexOp stk = popFlex(state, pc);
      ...
      case LdV: {
        ...
        localVarPo nv = markVarStart(state, constantFlex(state, voidIndex), pc);
        pushVar(state, pc, nv);
        continue;
      }
      case LdC: {
        // load literal from constant pool
        pushVar(state, pc, markVarStart(state, constantFlex(state, code[pc].fst), pc));
        continue;
      }
      case Ld: {
        // load stack from lcl[xx]
        FlexOp src = getLclSrc(state, code[pc].fst);
        pushVar(state, pc, markVarStart(state, src, pc));
        maybeRetireVar(state, code[pc].fst, pc + 1);
        continue;
      }
      case St: {
        // copy tos to local[xx]
        localVarPo lcl = findLocal(state, code[pc].fst);
        FlexOp vl = popFlex(state, pc);
        storeFlex(state, pc, vl, lcl->src);
        continue;
      }
      case StV: {
        // clear a local to void
        localVarPo lcl = findLocal(state, code[pc].fst);
        storeFlex(state, pc, constantFlex(state, voidIndex), lcl->src);
        continue;
      }
      case Tee: {
        // copy tos to local[xx]
      ...
      case TG: {
        ...
        armReg glb = getTempReg(state, pc);
        FlexOp vl = topFlex(state, pc);
      ...
      case Sav: {
        ...
        releaseReg(jit, tmp);
        pushVar(state, pc, markVarStart(state, RG(cel), pc));
        continue;
      }
      case LdSav: {
        ...
        pushVar(state, pc, markVarStart(state, RG(tmp), pc));
        continue;
      }
      case TstSav: {
        ...
        armReg tmp = getTempReg(state, pc);
        loadFlex(state, pc, sng,RG(tmp));
        armReg tr = getTempReg(state, pc);
        armReg fl = getTempReg(state, pc);
      ...
      case Get: {
        ...
        armReg vl = getTempReg(state, pc);

        loadFlex(state, pc, cel,RG(vl));
        ldr(vl, OF(vl, OffsetOf(CellRecord, content)));
        pushVar(state, pc, markVarStart(state,RG(vl), pc));
        continue;
      }
      case Assign: {
        ...
        FlexOp vl = popFlex(state, pc);
        armReg tmp = getTempReg(state, pc);
        armReg tmp2 = getTempReg(state, pc);
      ...
      case ICase: {
        armReg ix = getTempReg(state, pc);
        armReg divisor = getTempReg(state, pc);
        armReg quotient = getTempReg(state, pc);
        ...
        armReg off = getTempReg(state, pc);
        adr(off, jmpTbl);
        add(off, off, LS(ix, 2));
        br(off);
        releaseReg(jit, off);
      ...
      case IAdd: {
        ...
        armReg a1 = getTempReg(state, pc);
        armReg a2 = getTempReg(state, pc);
        ...
        mkIntVal(jit, a1);
        pushVar(state, pc, markVarStart(state, RG(a1), pc));
        releaseReg(jit, a2);
        continue;
      }
      case ISub: {
      ...
      case IDiv: {
        ...
        if (lbl != Null && tgtBlock->phiVar != Null) {
          storeToPhiVar(state, pc, constantFlex(state, divZeroIndex), tgtBlock->phiVar);
          b(lbl);
        } else
      ...
      case FAdd: {
        ...
        pushVar(state, pc, markVarStart(state, RG(reslt), pc));
        continue;
      }
      case FSub: {
        // L R --> L-R
      ...
      case Alloc: {
        ...
        pushVar(state, pc, markVarStart(state, RG(term), pc));
        continue;
      }
      case Closure: {
        int32 key = code[pc].fst;
...
armReg allocSmallStruct(codeGenPo state, int32 pc, clssPo class, integer amnt) {
  ...
  armReg h = getTempReg(state, pc);
  armReg c = getTempReg(state, pc);
  armReg l = getTempReg(state, pc);
  armReg reslt = getTempReg(state, pc);
...
void testResult(codeGenPo state, int32 pc, blockPo tgtBlock) {
  ...
  storeToPhiVar(state, pc, er, phiVar);
...
void invokeIntrinsic(codeGenPo state, int32 pc, runtimeFn fn, int32 arity, FlexOp args[]) {
  ...
  stashLiveLocals(state, pc, MAX_INT32);
...
static void stashLocal(codeGenPo state, int32 pc, localVarPo v) {
  if (v->live && isRegisterOp(v->src)) {
    storeFlex(state, pc, v->src, OF(AG, v->stkOff*pointerSize));
  }
}
...
static int32 stashLiveLocals(codeGenPo state, int32 pc, int32 cnt) {
  ...
  for (int32 ix = 0; ix < state->numLocals && cnt > 0; ix++) {
    ...
    if (var->live && var->varDesc->end >= pc) {
...
static void voidOutSlot(codeGenPo state, int32 pc, int32 offset) {
  for (int32 vx = 0; vx < state->numLocals; vx++) {
    ...
    if (var->live && var->stkOff == offset)
      return;
  }
  storeFlex(state, pc, constantFlex(state, voidIndex),OF(AG, offset*pointerSize));
}
...
localVarPo findPhiVariable(codeGenPo state, int32 pc) {
  localVarPo var = markVarStart(state, RG(XZR), pc);

  FlexOp src = getLclSrc(state, var->varDesc->varNo);
  var->src = src;

  return var;
}

void storeToPhiVar(codeGenPo state, int32 pc, FlexOp val, localVarPo phiVar) {
  storeFlex(state, pc, val, phiVar->src);
}
...
armReg getTempReg(codeGenPo state, int32 pc) {
  armReg tmp = findFreeReg(state->jit);
  if (tmp == XZR) {
