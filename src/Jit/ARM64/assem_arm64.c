...
static void
encodeDPReg2Src(uint1 wide, uint1 S, uint8 op1, armReg R1, uint8 op2, armReg R2, armReg RD, assemCtxPo ctx) {
...
void adx_reg(uint1 wide, armReg RD, uint1 sign, ExtendType et, uint8 shift, armReg S1, armReg S2, assemCtxPo ctx) {
  encodeDPReg2Src(wide, 0, 0x59, S1, ((sign << 5) | ((et & 0x3) << 3) | ((shift & 0x7))), S2, RD, ctx);
}
