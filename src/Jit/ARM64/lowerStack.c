...
retCode setupLocals(localVarPo stack, argSpecPo newArgs, int32 count,
  ...
  for (int32 ix = 0; ix < count; ix++) {
    ...
    switch (var->kind) {
    case inRegister: {
      newArgs[ix] = (ArgSpec){.src = RG(var->Rg),
    ...
    case isLocal:
    case inStack: {
      newArgs[ix] = (ArgSpec){.src = OF(AG, var->stkOff * pointerSize),
                              .dst = OF(AG, tgtOff * pointerSize),
                              .mark = True,
                              .group = -1};
      break;
    ...
    case isConstant: {
      newArgs[ix] = (ArgSpec){.src = OF(CO, var->key * pointerSize),
                              .dst = OF(AG, tgtOff * pointerSize),
                              .mark = True,
                              .group = -1};
      break;
...
void frameOverride(jitBlockPo block, int arity) {
  ...
  for (int32 ax = arity - 1; ax >= 0; ax--) {
    ...
    case inRegister: {
      tgtOff--;
      newArgs[ax] = (ArgSpec){.src = RG(var->Rg),
                              .dst = OF(AG, tgtOff * pointerSize),
                              .mark = True,
                              .group = -1};
      continue;
    ...
    case inStack: {
      tgtOff--;
      newArgs[ax] = (ArgSpec){.src = OF(AG, var->stkOff * pointerSize),
                              .dst = OF(AG, tgtOff * pointerSize),
                              .mark = True,
                              .group = -1};
      continue;
    ...
    case isConstant: {
      tgtOff--;
      newArgs[ax] = (ArgSpec){.src = OF(CO, var->key * pointerSize),
                              .dst = OF(AG, tgtOff * pointerSize),
                              .mark = True,
                              .group = -1};
      continue;
...
retCode propagateStack(jitCompPo jit, valueStackPo srcStack,
  ...
  int32 top = min(srcStack->vTop, tgtHeight);

  for (int32 v = 0; v < top; v++) {
