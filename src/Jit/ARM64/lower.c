  ...
  for (int32 pc = from; ret == Ok && pc < endPc;) {
    ...
    switch (code[pc].op.op) {
      ...
      case sAbort: {
        ...
        invokeIntrinsic(state, pc, pc + insSize, (runtimeFn) abort_star, 3, (FlexOp[]){RG(PR), RG(loc), val}, 0, (FlexOp[]){});
        releaseReg(jit, loc);
        pc += insSize;
        continue;
      }
      ...
      case sCLit: {
        ...
        int32 key = opand(1);
        FlexOp vl = localFlex(state, pc, opand(3));
        invokeIntrinsic(state, pc, pc, (runtimeFn) sameTerm, 2, (FlexOp[]){vl, constantFlex(key)}, 1, (FlexOp[]){RG(RTV)});
        cmp_w(RTV, IM(True));
        blockPo tgt = targetBlock(block, pc + opand(2), sBlock);
        bne(breakLabel(tgt));
        pc += insSize;
      ...
      case sFiber: {
        ...
        invokeIntrinsic(state, pc, pc + insSize, (runtimeFn) newStack, 3, (FlexOp[]){RG(PR), IM(True), lam}, 1, (FlexOp[]){RG(RTV)});
      ...
      case sLine: {
        ...
        if (lineDebugging) {
          ...
          invokeIntrinsic(state, pc, pc + insSize, (runtimeFn) lineDebug, 2, (FlexOp[]){RG(PR), constantFlex(locKey)}, 0, (FlexOp[]){});
        }
        pc += insSize;
        continue;
      }
