  ...
  for (int32 pc = from; ret == Ok && pc < endPc;) {
    ...
    switch (code[pc].op.op) {
      ...
      case sSG: {
        ...
        FlexOp tmp = localFlex(state, pc, opand(2));
      ...
      case sLdSav: {
        ...
        tst(tmp, RG(XZR));
        beq(breakLabel(targetBlock(block, pc + opand(2), sBlock)));
      ...
      case sStSav: {
        ...
        armReg tmp = findARegister(state, pc);
        loadFlex(state, pc, sng,RG(tmp));
        FlexOp val = localFlex(state, pc, opand(3));
      ...
      case sIDiv: {
        ...
        FlexOp tgt = localFlex(state, pc, opand(2));
        storeFlex(state, pc, RG(dividend), tgt);
      ...
      case sFAdd: {
        ...
        storeFlex(state, pc, RG(reslt), tgt);
        releaseReg(jit, a2);
        releaseReg(jit, a1);
      ...
      case sAlloc: {
        ...
        storeFlex(state, pc,RG(term), localFlex(state, pc,opand(2)));
        pc += insSize;
        continue;
      }
      case sClosure: {
      ...
      case sFiber: {
        ...
        storeFlex(state, pc,RG(X0), localFlex(state, pc,opand(1)));
        pc += insSize;
        continue;
      }
      case sSuspend: {
        ...
        invokeIntrinsic(state, pc, (runtimeFn) detachStack, 3, (FlexOp[]){
                          RG(PR), localFlex(state, pc,opand(1)), localFlex(state, pc,opand(2))
                        });
      ...
      case sResume: {
        ...
        invokeIntrinsic(state, pc, (runtimeFn) attachStack, 3, (FlexOp[]){
                          RG(PR), localFlex(state, pc,opand(1)), localFlex(state, pc,opand(2))
                        });
        ldr(X16, OF(STK, OffsetOf(StackRecord, pc)));
        br(X16);
      ...
      case sRetire: {
        ...
        invokeIntrinsic(state, pc, (runtimeFn) detachDropStack, 2, (FlexOp[]){RG(PR), localFlex(state, pc,opand(1))});
      ...
      case sUnderflow: {
        ...
        stp(RTV, RTS, PRX(SP,-16));
        invokeIntrinsic(state, pc, (runtimeFn) detachDropStack, 2, (FlexOp[]){RG(PR),RG(STK)});
        ldp(RTV, RTS, PSX(SP,16));
        ldr(X16, OF(STK, OffsetOf(StackRecord, pc)));
        br(X16);
        pc += insSize;
      ...
      case sLine: {
        ...
        if (lineDebugging) {
          ...
          invokeIntrinsic(state, pc, (runtimeFn) lineDebug, 2, (FlexOp[]){RG(PR), constantFlex(locKey)});
        }
        pc += insSize;
        continue;
      }
      ...
      case sdBug: {
        ...
        if (lineDebugging) {
          ...
          switch (code[npc].op.op) {
            case sAbort: {
              invokeIntrinsic(state, pc, (runtimeFn) abortDebug, 2, (FlexOp[]){RG(PR), constantFlex(locKey)});
              break;
            }
            case sEntry: {
            ...
            case sAssign: {
              invokeIntrinsic(state, pc, (runtimeFn) assignDebug, 2, (FlexOp[]){RG(PR), constantFlex(locKey)});
              break;
            }
            case sFiber: {
...
int32 loadArgsToRegisters(codeGenPo state, registerMap argRegs, int32 pcBase, int32 argBase, int32 arity) {
  ...
  int32 minOffset = stashLiveLocals(state, argBase); // save vars that will be live after the call
  voidOutFrameLocals(state, pcBase, minOffset); // void out gaps in the locals map

  for (int32 ix = 0; ix < arity; ix++) {
    FlexOp argSrc = sourceOperandFlex(state, pcBase, ix);
