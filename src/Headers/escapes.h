...
escape(_pkg_is_present, "|F(ss)lr"ERRCODE, "True if an identified package is available")
escape(_in_manifest, "|F(sss)lr"ERRCODE, "True if pkg/version/kind is present in manifest")
...
escape(_digitCode, "|F(c)ir"ERRCODE, "convert char to num")
...
escape(_str_hdtl, "F(s)"optionType("(cs)"), "pick up the first character and return remainder")
escape(_str_back, "|F(s)(sc)r"ERRCODE, "pick up the last character and return remainder")
...
escape(_getenv, "F(s)"optionType("s"), "get an environment variable")
