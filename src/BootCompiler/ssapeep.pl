...
vrRead(Vr,iBlock(Vrs,_)) :- is_member(Vr,Vrs).
...
vrWrite(Vr,iBlock(Vrs,_)) :- is_member(Vr,Vrs).
...
dropVar(Vr,[iLoop(In)|Is],[iLoop(Inx)|Isx]) :-
  dropVar(Vr,In,Inx),
  dropVar(Vr,Is,Isx).
dropVar(Vr,[iBlock(Vx,VI)|Is],[iBlock(Vx,VnI)|Isx]) :-
  dropVar(Vr,VI,VnI),
  dropVar(Vr,Is,Isx).
