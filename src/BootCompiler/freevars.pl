...
freeVars(implies(_,G,T),Ex,Q,F,FV) :- ptnGoalVars(G,Ex,E1),freeVars(G,E1,Q,F,F0),freeVars(T,E1,Q,F0,FV).
...
ptnVars(v(_,Lb,_),Q,Qx) :- addVar(Lb,Q,Qx).
...
ptnGoalVars(cond(_,T,L,R,_),Q,Qx) :-
  ...
  ptnGoalVars(L,Q0,Q1),
  ptnGoalVars(R,Q1,Qx).
ptnGoalVars(implies(_,G,T),Q,Qx) :-
  ptnGoalVars(G,Q,Q0),
  ptnGoalVars(T,Q0,Qx).
