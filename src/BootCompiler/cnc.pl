...
%  reportMsg("iterator in search has type %s",[ItrTp]),
  ...
  FnTp = funType(tplType([PtnTp,RsltTp]),MdlTp),
%  reportMsg("entangle iterator type %s with monad %s",[ItrTp,funType(tplType([SrcTp,MdlTp,FnTp]),MdlTp)]),
  sameType(funType(tplType([SrcTp,MdlTp,FnTp]),MdlTp),ItrTp,[]),
