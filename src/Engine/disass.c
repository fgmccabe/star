...
static ssaInsPo showOperands(ioPo out, ptrPo args, ssaInsPo pc, char *fmt);
...
case s##Op:{\
  outMsg(out, #Op);\
  return showOperands(out,args,pc+1,fmt);\
}
...
ssaInsPo showOperands(ioPo out, ptrPo args, ssaInsPo pc, char *fmt) {
  char *sep = " ";
  while (*fmt != '\0') {
    outMsg(out, sep);
    sep = ", ";
