...
DebugWaitFor enterDebugger(enginePo p, termPo lc) {
  ...
  switch (pc->op) {
    ...
    case Call:
      return callDebug(p, Call, lc, getConstant(operand(pc,0)));
    case XCall:
      return callDebug(p, XCall, lc, getConstant(operand(pc,0)));
    case TCall:
      return tcallDebug(p, lc, getConstant(operand(pc,0)));
    case OCall:
