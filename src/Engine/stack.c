...
void stackSanityCheck(stackPo stk) {
  ...
  assert(stk->fp >= stackFrameBase(stk) && ((ptrPo) (stk->fp + 1)) <= stk->sp);
...
void stackTrace(enginePo p, ioPo out, stackPo stk, integer depth,
  ...
  do {
    while (fp > stackFrameBase(stk) && maxDepth-- > 0) {
...
void attachStack(enginePo P, stackPo top) {
  ...
  top->bottom = Null;
  P->stk = bottom;
  recordTermUpdate((termPo)bottom); // We must mark resumed stacks
}
