...
static retCode setInterruptHandler();
...
retCode setInterruptHandler() {
  outMsg(logFile, "set up interrupt handler\n%_");
...
static void processQ() {
  struct timeval now;
  clearTimerInterrupt(); // clear any existing timer interrupt
...
retCode setupAlarm(struct timeval when, timeFun onWakeup, void *cl) {
  ...
  clearTimerInterrupt(); // Clear any existing timers
