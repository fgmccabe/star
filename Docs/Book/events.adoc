...
Many (most?) real world applications do not fit into the classical script mold
of doing something and finishing with some results. A compiler is a good example
of the latter: take some source code, parse it, check it and generate some
output code. However, most applications are more event driven: look for some UI
event and respond to it; wait for something to happen and respond to it.

In this chapter we look at some of Star's coroutining features -- such as `task`
structures, `async` functions and task schedulers. Together, these allow us to
construct so-called responsive applications.footnote:[Currently, Star does not
support multi-threaded applications.]

A `task` is a suspendable computation. That is, a task can be suspended and this
allows other ``task``s to resume their work. Tasks are performed in the context
of a task scheduler; this is responsible for managing a collection of tasks and
for deciding when to complete the scheduler. It is possible to have a tree-like
hierarchy of task schedulers; and starting a scheduler is a normal function
call.

One of the important features of task-oriented programming is the _event_. Task
schedulers are responsible for mapping events to individual tasks -- typically
the primary reason for waking up a suspended computation is to send it an event
that is related to the `task`.

@node Tasks
@section Tasks

A task expression denotes a possibly suspended _computation_ --
which itself is a sequence of actions -- and which may have a value
when the computation is performed. Furthermore, tasks have the
potential to be executed concurrently or even in parallel with other
tasks.

Like tasks, functions can also be viewed as representing suspended
computations. However, unlike functions, a task expression represents
a single suspended computation.

A task expression is written as a sequence of actions, enclosed in a
task block; for example, in:
...
the variable @code{T} is bound to a task expression whose returned
value will be 3 -- the value returned by the action:
@example
valis X+Y
@end example

Fiber expressions have a type: @code{task[t]} where @code{t} is the
type of the value returned by the task. So, for example, the type of
@code{T} above is given by
...
@node The value of a task
@subsection The value of a task

As we saw above, a task computation produces a value by performing the
@code{valis} action:
...
The value of a task value is obtained using the @code{valof} operator:

@example
valof T
@end example
denotes the value returned by the task computation @code{T}.

It is important to remember, in thinking about tasks, that there is
the extra level of indirection: the value of a task expression is a
computation. When the computation is performed, it too has a value.

Note that the @code{valof} operator also has the effect of forcing the
completion of the task @code{T}; in particular, the valof operator is
blocked until the task has completed and its value can be returned.

Although the @code{valof} operator denotes the end of the computation,
it does not denote the start of the computation.
...
Obviously a task that does not have a value does not require it's body
to contain a @code{valis} action; however, if it does, then that action
should be:
...
In effect, tasks that do not purport to return values _do_ return a
value -- the void tuple value. To avoid the clumsiness of handling
empty tuples, especially in the context of actions, it is better to
perform such a task instead. Like the valof expression form, the
perform action forces completion of its task argument -- thus
achieving some synchronization with the performed task.
...
Void tasks are occasionally useful -- for example, multi-tasked
applications often contain a _server_ component: a component that
responds to requests made by other tasks. Such a component typically
is not expected to return and so does not have a useful return value.

Note that it is possible to access a task value multiple
times. However, the actions in the task body will only ever be
performed once: subsequent attempts to get the value of the task
simply return the value previously computed.

@node Background tasks
@subsection Background tasks

A task can be started in the background -- i.e., concurrently with
other tasks -- by using the @code{background} operator, which is a function
from tasks to tasks. It's type is given by
@example
background:all t ~~ (task[t])=>task[t].
@end example

Applying @code{background} to a task not only yields a task of the
same type, but also one yielding the same value.

The @code{background} operator has a simple effect -- it starts its
argument task so that it will operate in parallel with other
computations.

If it happened to be the case that a backgrounded task already
completed, then the @code{background} operator has no effect.

@node Computing with tasks
@subsection Computing with tasks

Like other values, tasks are first class: they can be assigned to
variables, kept in data structures, passed to and from functions and
so forth. This flexibility leads to great expressive power -- many
patterns of computations can be readily encoded as task-valued
functions.

For example, consider the @code{mp} function -- which is a facsimile
of the standard @code{fmap} function specialized for list values:
...
In this @code{mp} function,footnote:[Notice that we do not need to
mark the sequence expressions as list sequences -- type inference
takes care of this for us.] the functional variable @code{F} denotes
the computation to be applied to each element of the input
list. Suppose that each computation of F were non-trivial, and we
wanted to spread the load across multiple cores -- i.e., to perform
the map operations in parallel. The task notation, in particular
background tasks will help us to achieve that.

Recall that @code{fmap} is a standard function that is defined in the
functor contract and is implemented for many different collection
types.

We will first of all transform @code{mp} to use tasks rather than
...
As it stands, this function has very similar performance
characteristics to mp; except that we are using the task expression
notation. In order to run the different elements in parallel we need
to also use the background operator:
...
This program computes each element of the result in a separate
background task. However, it is not a true parallel fmap because we
wait for each element before continuing to the next element.

A better approach is to first of all construct a list of tasks and
then to separately collect their values in a second phase:
...
This function does not wait for any task to complete until they have
all been spun out into background activities. This gives the maximum
opportunity for the independent tasks to complete before we actually
need their values.
...
So, a more idiomatic way of writing the parmap function is to make it
a task valued function; we also generalize away from the specific list
collection type and use the standard fmap function to allow for any
collection type:
...
@node Computation expressions
@section Computation expressions

Fiber expressions are instances of a more general @strong{Star} feature: the _computation expression_. Fiber expressions are oriented towards the concurrent execution of computations; other forms of computation expression have different purposes. In particular, the simple valof expression:
...
There are several forms of computation expression in the @strong{Star} library; they are used for expressing tests for example. In fact, computation expressions are _syntactic sugar_ for _monadic_ expressions -- and play the same role in @strong{Star} as the do notation plays in Haskell.
...
The AA function becomes the somewhat more complex form:footnote:[This
is actually a somewhat sanitized version of the raw code. This is
because we have hidden a required transformation from action sequences
to functional expressions that encode the computation's ordering as
function calls.]
...
It is beyond the scope of this book to explore more deeply the handling of monadic computations. However, if you wish to explore further, a more complete description of computation expressions is given in the @strong{Star} language definition.
...
@node A rendezvous is a future event
@subsection A rendezvous is a future event
...
@node Channels of communication
...
However, it is also true that synchronous communication is essentially impossible when that communication involves multiple computers. The concurrency features in @strong{Star} support multiple threads of activity within a single application but do not directly support networked applications. We believe that, for other reasons, such networked applications are better served with different features -- features that directly address issues involving agenthood. We shall see some of these in our treatment of [Concurrent Actors][concurrent-actors].
...
In our message passing sequence, we actually do _not_ want the task arguments to complete in a sequential order -- so ignoreing (sic) the task is the appropriate command.

@node Computing primes with tasks
@subsection Computing primes with tasks
...
[eras13]: eras13.png width=385px height=174px

[eras17]: eras17.png width=436px height=158px

@node Philosophers have to eat too
@subsection Philosophers have to eat too
...
Of course, it is only a little eccentric that the philosophers eat
with two forks rather than a knife and fork.
...
[phil]: phil.png width=312px height=295px

#### Using channels and rendezvous to implement semaphores

A semaphore is a general structure for implementing mutual exclusive access to some resource.

@strong{Semaphore}

A @strong{semaphore} is a variable, with associated @strong{grab} and @strong{release} operations, that can be used to manage access to a shared resource by multiple concurrent activities.
...
The idea is, of course, that if the resource is not currently available then activities that attempt to @strong{grab} the semaphore will be blocked. When the resource becomes available (by someone else performing the semaphore's @strong{release} action) then the @strong{grab} may be re-attempted.

Semaphores are useful, but they are prone to mis-programming. Use of a semaphore relies on the programmer ensuring that the @strong{release} is invoked after a @strong{grab} and, furthermore, any connection between the semaphore and the governed resource is purely implicit. Both of these are common sources of errors in concurrent programs.

To build a semaphore we have to arrange for a unique resource[^cf1] together with coordinated access to it; preferably without exposing the resource itself (to prevent it from being _contaminated_ by external code).
...
In implementations of @strong{Star} that do not support tail recursion, this code would need to be rewritten as a loop. However, we leave it as a recursion for expository purposes.
...
[^cf1]: Not to be confused with the resource that is governed by the semaphore.
...
The queue data type is a builtin type to @strong{Star}. It implements a queue functionality allowing elements to be posted at one end and removed from the other. Like other collection types, the stream contract is implemented for it; which means that we can use sequence notation when writing queue expressions and patterns.
...
so, apparently, we take the value of the task returned by the wait for
and rewrap it back as a task.
...
The true merit of @strong{Star}'s concurrency features is that they do allow these higher level constructions to be programmed in a way that does not require extreme effort.
