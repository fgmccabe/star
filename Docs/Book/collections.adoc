...
expression ``X%2==0`` amounts to a test that ``X`` is even (its
...
@node The ``filter`` contract
@subsubsection The ``filter`` contract
...
_projection_. For example, if we wanted to compute the average
...
@quotation
Of course, it still would not count as the _most efficient_
