...
It is quite possible to unpack this definition into many important aspects; for
example we can distinguish _permissive_ policies from _audit_ policies. 
...
Functions such ``lendingIsPermitted`` is not typically hard coded into lending
applications, nor is it included in the input data (the user's name and the book
they want to check out).footnote:[Although the user might have presented
additional input -- such as a special pass -- that the application can use to
override its own reluctance.]
...
NOTE: In larger scale applications, having an explicit policy oracle can be a
useful architectural technique as it can help factorize out what a system can do
from the mechanisms needed to achieve its goals.
