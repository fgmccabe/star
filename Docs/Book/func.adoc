...
Closure:: A closure is a structure that is the value of a function
expression and which may be applied to arguments.
...
``(X)=>X==S`` lambda expression with named functions. The reason is
that the lambda is _not_ defined in the same way that named functions
are defined -- because it occurs as an expression not as a
statement. If we wanted to define a named function which also captures
``S``, we would have to be able to define functions inside
expressions.

There is an expression that allows us to do this: the ``let``
expression. A ``let`` expression allows us to introduce local
definitions anywhere within an expression. We can define our lambda as
the named function ``isS`` using the ``let`` expression:
...
Star actually has _two_ forms of let expressions -- the one we have
used so far does not allow function definitions to be recursive -- and
...
----
factorialC:all t,e ~~
  arith[t],coercion[integer,t->>e] |= (t)=>t throws e.
...
Of course, coercion is also governed by contract, a fact represented in the type
signature by the coercion contract constraints on the type of ``t`` -- and the
implicit exception type ``e``.

In any case, using these techniques, it is possible to write numeric functions
without unnecessarily committing to specific number types. That in turn helps to
...
Finally, we can pull the value out of an optional value using the
unary form of `"?"`:
...
NOTE: that this is actually mediated via the `pull` contract. The
intermediate step in this reduction is:

[source,star]
----
pull_(.some(43))
----
and `pull_` is defined for `option` values to extract the content.footnote:[The `pull` contract is also defined for `either` values.] In
the case of pulling from `option` values, any attempt to pull from
`.none` results in an `exception` being thrown.

Overall, the ``option`` type is part of an elegant approach to
nullability that is easily incorporated into the type system. However,
it is not the only Star feature oriented to _failable_ computation.

== Exceptions and short circuits

Not all computation goes according to plan; some computations can
fail. The simplest examples of this are in arithmetic: division by
zero is not defined, and taking the square root of a negative number
is not defined over the real numbers. Other examples typically involve
something to do with input/output.

Star has a so-called exception handling mechanism. However, the
inspiration of it owes more to using optional values than the recovery
from errors.
...
* The type checker is able to verify that the type of the `_Ptn_` in
  the catch handler is consistent with exceptions that might be thrown
  in `_Exp~1~_`.
* Furthermore, the type of `_Exp~2~_` must be the same as that of
  `_Exp~1~_` and is the type of the whole expression.
* Only one type of exception may be thrown within `_Exp~1~_`.

The way to think about expression forms like `try` catch` (and their
analogous actions) is that the form allows a kind of short circuit
evaluation: if the body of the `try` `catch` throws an alternate value
then the overall evaluation if short circuited and evaluation
continues with the catch handler.

NOTE: This is somewhat restricted compared to exception handling found
in other languages. However, this restriction has benefits also.

One benefit is that exceptions are fully integrated into the type
system. We can, for example, have functions that throw exceptions, as
in the definition of the division operator itself:
...
types defined using algebraic type definitions, generic types,
function types and even function types that can throw. Recall also
that we only require programmers to explicitly declare the types of
quantified variables and functions. It is worth pausing a little to
...
_Unification_:: An algorithm that replaces variables with values in
such a way as to make two terms identical.
...
equal by _binding_ the type variable _t~N~_ to ``integer``.

We initially picked the type of ``N`` to be an arbitrary type
variable, but the process of checking consistency leads us to refine
this and make the type concrete. I.e., the use of ``N`` in a context
where an integer is expected is enough to allow the compiler to infer
that the type of ``N`` is indeed ``integer`` and not _t~N~_.
...
the effect of introducing _constraints_ on types based on which
...
A sharp-eyed reader will notice that Star's type system is different
in nature to that found (say) in OO languages. In Star's type system,
types are considered to be consistent only if they are _equal_. This
is quite different to the notion of consistency in OO languages where
an argument to a function is consistent if its type is a _sub-type_ of
the expected type.
...
variables that are _not_ quantified. In fact, it is fairly
...
----
concat(.nil,x)=>x.
concat(.cons(h,tl),x) => .cons(h,concat(tl,x))
----
a generalized type for ``concat``:
[source,star]
----
concat:all t ~~ (cons[t],cons[t])=>cons[t].
----
...
this policy would require the ``concat`` program above to be written:

[source,star]
----
concat:all t ~~ (cons[t],cons[t])=>cons[t].
concat(.nil,x:list[t])=>x.
concat(cons(h:t,tl:cons[t]),x:cons[t]) => cons(h,concat(t,x))
----
The design of Star aims to strike a balance between useability and rigor:
...
[aside]
If the type of a variable is known from context, then use that
type to verify the type of any value the variable may be
bound to. Otherwise, use type inference on the value to infer a type
for the variable but do not attempt to generalize it by adding
quantifiers to the inferred type.

We are only able to scratch the surface of the type system here. It is
certainly true that -- like many modern functional languages -- Star's
type system is complex and subtle. The primary motivation for this
complexity is to reduce the burden for the programmer: by being able
to infer types automatically, and by being able to address many
...
to functional programming than can be captured in a few pages.
