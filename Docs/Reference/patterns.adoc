...
<<equalityContract>>); and therefore, the pattern may not be valid if `equality`
is not implemented for the type of `X`.
...
necessitates the extension: i.e., control _must_ flow from the pattern
