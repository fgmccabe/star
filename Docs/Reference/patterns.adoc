...
(((patterns)))
Patterns are templates that are used to match against a value; possibly binding
one or more variables to components of the matched value. Patterns are used as
guards in equations and rules, as filters in query expressions and in `for`
loops. Patterns represent one of the fundamental mechanisms that can guide the
...
A repeated occurrence of a variable in a pattern, or the use of a variable that
is already in scope, is equivalent to a call to the `==` predicate. For example,
the pattern:
...
<<equalityContract>>); and therefore, the call and the pattern may not be valid
if `equality` is not implemented for the type of `X`.
...
The scope extension of a pattern depends on where the pattern occurs. However,
as a general rule, variables defined in a pattern extend outward and to the
right of where the pattern variable occurs -- so long as the pattern
necessitates the extension: i.e., control flow _must_ flow from the pattern
variable throughout the scope extension.

For example, if the variable pattern is part of a constructor pattern, then the
concstructor pattern is part of the scope extension of the variable. Similarly,
a variable in a pattern on the left of a conjuction is also in scope in the
right hand part of the conjunction.

On the other hand, if the variable occurs on the laft hand side of a disjuction,
then the variable's scope extension does not include the right hand part of the
disjunction.
