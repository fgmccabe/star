...
NOTE: Note that `::` coercions may throw an exception -- depending on the
signature of the specific implementation of the coercion. This reflects the
potential for coercions to fail.
+
...
The `:?` form of coercion expression can be used to obtain an ``option``al value
from a potentially failable coercion. If the coercion fails ( throws an
exception) then the value will be `.none`; otherwise the successful result is
returned as a `some`. For example, the coercions of `string` values to `integer`
is not guaranteed -- it depends on whether the string denotes an integer or not:
[source,star]
----
"fred" :? integer
----
