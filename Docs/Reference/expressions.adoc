...
Procedure rules are the procedure analog of lambda expressions: they denote an
anonymous procedure. Procedure rules take the form:

.Procedure Rule
[source,star]
----
procedureRule --> tuplePattern, ["do"], action |
...
(X) do X+Y
----
Procedure rules may appear anywhere a procedure value is permitted.
...
Procedure rules are somtimes used as part of a visitor pattern. For example,
...
NOTE: Like lambda expressions, procedure rules may reference free variables.
