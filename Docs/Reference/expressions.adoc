...
(((algebraic literal)))
An <<algebraicLiteral>> denotes a value built from an algebraic type
constructor -- either a <<tupleLiteral>>, whose shape is purely
positional, or a <<constructorLiteral>>, whose shape is determined by
a constructor defined in a type definition.

[#algebraicLiteralFig]
.Algebraic Literal
[source,star]
----
algebraicLiteral --> tupleLiteral.
algebraicLiteral --> constructorLiteral.
----
...
Lambda expressions denote so-called anonymous functions. For example, the expression:
[source,star]
----
(X) => X+Y
----
is an anonymous function that doubles its argument.
...
Lambda procedures are the procedure analog of lambda expressions: they denote an
anonymous procedure. Lambda procedure rules take the form:

.Lambda Procedure Rule
[source,star]
----
lambdaProcedure --> tuplePattern, ["do"], blockAction |
  tuplePattern, blockAction.
----

Lambda procedure rules may appear anywhere a procedure value is permitted.

For example, a procedure rule to accumulate to an accumulator would
be:
[source,star]
----
(X) do { Acc := Acc! + X }
...
Lambda procedures are somtimes used as part of a visitor pattern. For example,
given the `visitCons` procedure in:

[source,star]
----
visitCons:all e ~~ (cons[e],(e){}){}.
visitCons([],_) do {}.
visitCons([E,..Es],P) do{
  P(E);
  visitCons(Es,P)
...
we can show all the elements of a list with the procedure call:

[source,star]
----
visitCons(L,(X){ show X })
----

NOTE: Like lambda expressions, lambda procedures may reference free variables.
...
----
memFib(N) => let{
  fibs : vector[thunk[integer]].
  fibs = { $$(fb(ix-1)+fb(ix-2)) | ix in range(2,N,1) }.
  fb(0) => 1.
  fb(1) => 1.
...
[source,star]
----
valofExpression --> ["valof"], action.
----

The `valof` expression uses an action to determine the value of the
expression. The effect of this is to perform the action and return its value as
the value of the `valof` expression.

At least one of the actions executed within the action must be a
<<valisAction>>. If the action completes with no `valis` action being executed,
execution of the entire program halts.
...
[source,star]
----
collectExpression --> ["collect"], action.
----

The `collect` expression performs the action and returns a collection of elements -- one for each time
the <<elemisAction>> is performed within the action.
...
NOTE: The `collect` expression does not have enough structure to fully determine
the type of the resulting collection. Therefore, it may be necessary to use a
<<typeAnnotation>> to constrain the result type.
...
typedExpression --> typeAnnotatedExpression.
...
casting in Star.] The type annotation is a way for the programmer to communicate
exactly what the expected type of an expression is -- there is no implied
coversion of values from one type to another.

NOTE: Type annotations are not commonly required; with the major exception being
<<sequenceLiterals>>. Sequence literals require explicit type information
because, without that the `sequence` contract that is used to construct sequence
literals cannot be properly resolved.
+
However, in many cases the type of a sequence literal expression can be properly
inferred -- due to the context surrounding the expression.

NOTE: The type annoted expression is shown as being enclosed in
parentheses. This is actually an artifact of the priority of the colon operator
relative to other commonly used operators in type expressions.
...
(((type coercion)))
A type coercion expression amounts to a call to convert a value of one type to
that of another.  For example, coercing a `float` value to an `integer`
value. In general, type coercions have the potential to change the value (for
example, by stripping any fractional part of the value) and/or fail.

Type coercion is supported by a special `coercion` contract (see <<coercionContract>>):

[source,star]
----
all s,t,e ~~ contract coercion[s,t->>e] ::= {
  _coerce:(s) => t throws e
}
----

where `s` denotes the source type, `t` the target type and `e` a potential
exception type.
...
which has the effect of declaring that the expression has type `integer` and the
`_coerce` function is an overloaded function that references a type-specific
implementation -- based on the source type of `X` and `integer`.

NOTE: Note that if the `e` type in the implementation is `void` then the type
checker verifies that no exception is possible, and uses of `::` do not need to
be enclosed in an exception handling context.

Not all coercions can fail, for example
[source,star]
----
"hello" :: cons[char]
----
is guaranteed to succeedfootnote:[Unless, of course, memory is exhausted.] --
because strings are essentially lists of characters.

The `:?` form of coercion expression can be used when the programmer can 'live
dangerously. If the coercion fails, then the program will be aborted.

There are many standard coercions available.  However, it is also possible for a
programmer to define their own type coercion by appropriately implementing the
`coercion` contract.
...
A zeta expression is used in situations where it is necessary to suppress some
of the normal type inference mechanisms:
...
the recorded type for `id` is `all x ~~ (x)=>x`. However, an occurrance of `id`,
as in:
...
There is a related similar notation -- the <<sliceExpression>> --
