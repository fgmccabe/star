...
NOTE: Note that if the `e` type in the implementation is `void` then the type checker verifies that no exception is possible, and uses of `::` do not need to be enclosed in an exception handling context.
