...
The type of a grammar non-terminal is written:
...
An optional non-terminal is written using a prefix ``?`` operator. An optional
is satisfied if either the argument is satisfied, or it is empty.

Note that, if an optional has an out, then it is _wrapped_ into an ``option``
value.

For example, if ``decimal`` has an ``integer`` output, then
...
has an output of type ``option[integer]``: if a ``decimal`` is parsed, and
produces ``42`` (say) then the output f ``? decimal >> D`` is ``.some(42)``.

If there is no ``decimal`` in the input, then the output will be bound to
``.none``.
...
(((grammar,invoking)))
A <<grammarCall>> is an expression that consumes a stream and returns an
optional value that represents the output of the grammar over the
stream. Typically, we guard such a call using an option match.

[#grammarCallFig]
.Grammar Call
[source,star]
----
grammarCall --> expression, ["-->"], expression.
----

The left-hand expression denotes the non-terminal -- and any arguments it takes --
to invoke; the right-hand expression denotes the stream, typically a `string`, to
be parsed.
...
we can use ``int`` to parse a string with:
...
which results in ``Ix`` being bound to ``123``.

If the invoked grammar does not actually return a value explicitly, then it will
return the unit tuple: ``()``. If the grammar fails, then the ``-->`` form
returns ``.none``.
