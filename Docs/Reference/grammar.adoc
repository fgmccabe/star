...
A grammar call is an expression that consumes a stream and returns an optional
value that represents the output of the grammar over the stream. Typically, we
guard such a call using an option match.
...
we can use ``int`` to parse a string with:a
