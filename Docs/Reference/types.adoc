...
For example, a procedure of two arguments -- an `integer` and a
`float` has a type that
takes the form:
[source,star]
----
(integer,float){}
...
signals that the procedure can throw an exception -- of type `string`
in this case -- when called. As with function types, it is possible for the exception type to
be quantified, as in:
