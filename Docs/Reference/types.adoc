...
functionType --> tupleType, ["{}"], ["throws"], type.
----
...
There are variants of the function type for procedures -- where there is no return type. A type of the form:
...
corresponds to the type of a two-argument procedure that takes an `integer` and
`string` argument -- and does not return anything.
