...
(((type system))) Star is a strongly, statically, typed language. That means
that all values and all variables must have a unique well-defined type that is
determinable by inspecting the text of the program -- effectively at _compile
time_.footnote:[The Star runtime also erases all type information: there is no
...
identifier `string` denotes the collection of all strings. More explicitly, a value has
type `string` iff footnote:[The term "iff" means "if and only if".] it belongs
to the collection denoted by the symbol `string`.
...
denotes the type expression associated with lists whose elements are all string
values. Other examples of `cons` type include lists of integers:
...
The ``void`` type is uninhabited: there are no values of type `void`. It is most
often used to signal cases where something is not going to happen: most
particularly with exceptions.

A type expression of the form:

[source,star]
----
(integer)=>string throws void
----

denotes a function that takes an `integer`, returns a `string` and does not
`throw` an exception. I.e., it is equivalent to just:
...
NOTE: The `void` type is not normally used explicitly. However, if a `contract`
defines a function that may throw exceptions, we can use `void` to indicate, for
example, implementations that doe not result in exceptions.
...
There are several main forms of structural type: tuple types, function and
procedure types, and record types.
...
NOTE: a function that returns `()` is not the same as a procedure -- which does
not return anything.

==== Singleton Type Tuple

(((tuple,singleton tuple type))) A singleton tuple must be written with two
parentheses. This is to disambiguate such terms from the use of parentheses to
express expression grouping. A type expression of the form:
...
NOTE: The double set of parentheses is not needed, for example, when giving
...
Record types are used as the basis of other features of the type language --
including record constructors and <<contract>>s.

Two record types are equivalent if their elements are pair-wise by name
equivalent. Note that the _order_ of elements is not important. For example,
given the types:
...
It takes the form of a possibly empty sequence of argument types -- denoting the
types of the arguments to the function -- enclosed in parentheses; followed by
the result type of the function. <<functionTypeFig>> highlights the form of the
function type:
...
(((procedure type)))
(((type,procedure)))
A procedure type denotes a procedure value. There are two forms of
procedure type: a normal, non-throwing, form and a throwing form. The
latter signals that the procedure may throw an exception, whereas the
non-throwing form is not permitted to throw an exception.

A procedure type takes the form of a possibly empty sequence of argument types
-- denoting the types of the arguments to the procedure -- enclosed in
parentheses; followed by a pair of empty braces. This syntax emphasizes the fact
that procedures do not return values.

<<procedureTypeFig>> highlights the form of the procedure type:

[#procedureTypeFig]
.Procedure Type
[source,star]
----
procedureType --> tupleType, ["{}"].
procedureType --> tupleType, ["{}"], ["throws"], type.
----

For example, a procedure of two arguments -- an `integer` and a `float` has a
...
[#throwingProcedureType]
A throwing procedure type, such as:
[source,star]
----
(integer,string){} throws string
----

signals that the procedure can throw an exception -- of type `string` in this
case -- when called. As with function types, it is possible for the exception
type to be quantified, as in:
...
This is a generic procedure type that takes an `x` and which potentially throws
an `e`.
...
The left hand side of a constructor type should either be a <<tupleType>> or a
<<recordType>> -- depending on whether the denoted constructor is a term
constructor constructor or a record constructor.

TIP: Explicit <<constructorType>>s are most used in the context of the
signatures of _abstract data types_: where a type and its constructors are
_exported_ from a record.
...
(((type,constraints))) There are three kinds of type constraint: contract
constraints, field constraints and implicit binding constraints.
...
It is possible combine different type constraints; for example, a type variable
could be constrained to implement multiple contracts, and even be combined with
a field constraint.
...
It is possible for contract constraints to reference more than one type. For
example, the standard `coercion` contract (see <<coercionContract>>) references
three types. A `coercion` `contract` constraint will therefore look like:
...
where `T1` represents the source type of the coercion, `T2` represents the
destination type and `X` represents the type of any exception thrown by the
coercion.

If the `-{zwsp}>>` clause is used, then the contract being referenced must have
a _functional dependency_ (((functional dependency))) associated with it.
...
The `-{zwsp}>>` clause identifies which type(s) are dependent on the type
argument(s) of the <<contract>>. (See <<ContractFunctionalDependency>>).
...
(((type,field))) (((type,constraints!field))) A _field constraint_ is a
requirement on a variable that if it is bound, it is bound to a record type with
an identified attribute.
...
in type language, quantifiers refer to types and apply to type expressions.

NOTE: The compiler will infer the type of expressions; but does _not_ infer any
quantified type. Functions that are intended to be generic must have explicit
type annotations associated with them.
...
quantification --> ["all"], boundTypes, ["~~"].
quantification --> ["exists"], boundTypes, ["~~"].
----

[#universalType]
==== Universally Quantified Types

(((type,universally quantified))) (((universally quantified type))) A
universally quantified type denotes a type that is valid for all substitutions
of a type variable.
...
The compiler will infer the type of expressions; but does _not_ infer any
quantified type. Functions that are intended to be generic must have explicit
type annotations associated with them.
...
The fact that within the record the type `el` is identified as `integer` does
not escape the record itself. Externally, the existence of the `el` type is
known but not what it
is.

It is permissible to refer to the type within the record by a dot reference --
otherwise known as an <<encapsulatedType>>.

[#encapsulatedType]
===== Encapsulated Type

(((encapsulated type))) (((type,encapsulated in record))) (((existential type)))
(((heterogenous types))) An `encapsulatedType` is a reference to a type that is
...
The actual type identified with an encapsulated type expression is strictly
opaque: it is assumed to be different to all other types. Which means that
effectively _only_ the other fields of the record variable `R` contain functions
and values that can be used in conjunction.
...
There are two forms of `boundType`, a simple type variable and a second form
that includes an arity.

The first form of `boundType` introduces a regular type variable -- i.e., a
variable which may be bound to any type. The second form is used to introduce a
...
denotes a quantified variable which may only be bound to type constructors that take one
argument -- for example `cons`.

WARNING: A regular type variable only unifies with regular types, and a type
constructor type variable only unifies with type constructors of the same arity.
...
The type variable `t` may be bound to a type expression such as `cons[string]`
but not to a higher-kinded type (such as `cons` itself).
...
NOTE: If the anonymous type occurs in a quantified type then it will denote a
new type whenever the quantified type is _refreshed_. The effect would be as
though there was a different anonymous type for each possible binding of the
quantifiers.
...
algebraicTypeDefinition --> quantification, algebraicTypeDefinition.
...
A constructor is a specification of a value of a type; i.e., constructors _paint
a picture_ of the shape of potential values of the type.
...
[source,star]
----
recordTypeDefinition --> typeTemplate, ["::="], identifier, recordType.
recordTypeDefinition --> quantification, recordTypeDefinition.
----
...
typeAliasDefinition --> quantification, typeAliasDefinition.
----
...
defines the `pair` type -- which has two type arguments -- as an alias of a
tuple type.

Note that any type variables on the right hand side of a type alias statement
must also have been mentioned on the left hand side.
...
typeExistsDefinition --> quantification, typeExistsDefinition.
----
...
where `_VN~t~_` is the expression that is on the right hand side of the `implementation` statement.
...
* Ensure that, for every implicit binding constraint of the form ``(_V_ : _T_)`` a
  variable `_V_` of type `_V_~t~` is in scope,
