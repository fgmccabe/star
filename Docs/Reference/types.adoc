...
(((nomical types))) (((types,nominal))) A nominal type is a term that identifies
a class of values by name. The name may or may not have type arguments -- in
which case, the type is said to be _parameterized_ and the name itself is
referred to as a _type constructor_.
...
One of the other differences between structural and nominal types is that the
latter may be used to denote _recursive_ types, whereas the former cannot.
...
simple types are pre-defined, <<definedSimpleTypes>> gives a table of
such types.
...
type constructor applied to one of more <<type>> arguments. For
example, the standard `cons` type constructor has one type
argument -- the type of elements of the `cons`.

[#typeConstructor]
A type constructor is a function over types. It is a mapping
from types to types.  Type constructors have a _type arity_ -- the number of
type arguments it expects. This is defined when the type itself is defined. It
is an error to write a type expression involving an incorrect number of type
arguments.
...
<<comparisonPredicates>> and <<arithContract>>).
...
A <<contract>> is a requirement on a <<type>> -- or tuple of types -- that
whatever type it is, that there must exist an `implementation` of the contract
for the type.
