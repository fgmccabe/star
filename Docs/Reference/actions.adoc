...
(((action types)))
(((action!types)))
The <<procedureTypeFig>> captures the intended shape of an action: an action is
performed for effect and does not return any value. However, actions _may_
return a value if they are syntactically part of a <<valofExpression>> -- or
part of a <<collectExpression>>.
...
The `{}` action is useful when you don't want to do anything.
...
The empty action block -- `{}` -- is equivalent to the <<nullAction>>.
...
<<tryCatchAction>>, a <<tryCatch>> expression or the enclosing function or procedure must have
been declared as throwing a value of the appropriate type.

NOTE: If evaluating the expression that denotes the exception to raise itself
raises an exception, that is a fatal error and causes overall execution of the
program to stop.

NOTE: There is no runtime search for a handling form: every `throw` must be lexically
accounted for.
...
(((let action))) (((action,let))) The `let` action allows an action to have
local definitions embedded within it. It plays the same role as let expressions
do (see <<letExpression>>).
...
caseActionRule --> ["|"], pattern, ["do"], action.
caseActionRule --> ["|"], pattern, ["default", "do"], action.
----
