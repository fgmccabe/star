...
NOTE: The rules in <<#procedureFig>> are written assuming that the procedure's
name is not also an operator. If it is, then the head of the rule will take the
infix form.
...
Procedures can be defined in a let expression, in recursive records, in packages
-- or a <<procedureRule>> can be used to craft a procedure literal directly as
an expression.
...
Each rule consists of a <<tuplePattern>> that is used to match
the call to the procedure and an action to perform. The left hand side of the
procedure may also have a guard associated with it, this guard may use variables
introduced in the pattern.
...
(((theta environment)))
Procedures are defined in the context of a scope -- for example, in the body of
a `let` expression (see <<letExpression>>), or at the top-level of a `package`.
...
* Type inference will not result in a generalized
  contract constraint. 
* If a procedure is verified against a type declaration then some additional
  rules will be applied during type checking:
** a universally bound type variable may not be constrained further by the text of the procedure definition.
** an existentially bound type variable may be constrained by the text of the
 procedure. 
...
As with functions and equations, a procedure may be defined with multiple rules.
Only one of the rules in the definition may apply.
...
(((procedure application,evaluation order)))
As with function calls, there is _no_ guarantee as to the order of evaluation of
arguments to a procedure application.

[#throwingProcedures]
==== Procedures that throw exceptions
(((exception, throwing procedure)))

A procedure whose type is a throwing type, (see <<throwingProcedureType>>), may
return an exceptional value rather return normally. For example, this
definition of `check` will throw an exception -- encoded as a `string` -- when
its argument is false:
...
NOTE: Like functions that throw, we must use explicit type signature to indicate
that `check` throws an `exception`.
