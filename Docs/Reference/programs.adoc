...
There are two kinds of main: a function or procedure with
name `main` that may have zero or more arguments; or a function or
procedure with name `_main` with one argument that is a `cons` list of
``string``s.
...
overall -- to the operating system.

If a package has a definition for `main`, but does not have one for
`_main`, then the command line arguments are coerced according to the
explicit type signature for `main`.
