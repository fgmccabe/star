...
In this chapter we outline the general standard contracts.

Many of these contracts have multiple implementations, for different
types and type combinations.
...
The second argument to `frmt` is a formatting string that is in a
...
on a so-called _picture_ style format string. For example, the format string for
formatting integers as dollars and cents is
...
type-specific; for example, for ``string``s, the ``size`` of a ``string`` is the
number of characters (Unicode code points) in the string.
...
implementation of `indexed` is __applicative__ in nature.
...
[NOTE]
====
Another way of differentiating left and right folds is by when the folding
function is applied: in a `leftFold`, the function is applied during the
recursive iteration of the sequence, whereas with a `rightFold` the function is
applied on exit from the recursion.

The distinction is not applicable to all the types that `folding` is
implemented for. For example, `leftFold` and `rightFold` are equivalent for the
`set` collection.
====
...
declaration should mark the dependent type as `void`. For example, it is always
possible to generate a `cons` list of `char` values from a `string`:
