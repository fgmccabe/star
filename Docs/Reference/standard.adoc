...
NOTE: Implementations should honor the condition that duplicates are eliminted in collections.
...
The `\-` function returns a new collection with an element removed.
...
The `pull` contract provides a function that can be used to extract an element
...
Since it is not always guaranteed that it is possible to extract a wrapped
value, the `pull_` function can throw an exception. However, the type of the
exception is determined by the implementation of the contract.

The `pull` contract is defined for the `either` type and the `option` type. For
example, the implementation for `either` looks like:
...
NOTE: In this case the `other` arm of the `either` value is used when `pull_`
fails to unwrap an `either` value.
